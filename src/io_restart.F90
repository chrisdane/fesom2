...
subroutine ini_ocean_io(year, mesh)
  ...
  integer                   :: ncid, j
  integer                   :: varid
  integer                   :: end_num_tracers = 2 ! always T/S
...
!!PS   call oce_files%def_node_var('ssh_rhs', 'RHS for the elevation', '?',   ssh_rhs, mesh)
  ...
  if (trim(mix_scheme)=='cvmix_IDEMIX' .or. trim(mix_scheme)=='cvmix_TKE+IDEMIX') then
        call oce_files%def_node_var('iwe', 'Internal Wave energy', 'm2/s2', tke(:,:), mesh)
  endif 
...
subroutine ini_bio_io(year, mesh)
    ...
    !===========================================================================
    !___Define the netCDF variables for 2D fields_______________________________
    call bio_files%def_node_var('BenN',    'Benthos Nitrogen', 'mmol/m3',   Benthos(:,1), mesh);
    call bio_files%def_node_var('BenC',    'Benthos Carbon',   'mmol/m3',   Benthos(:,2), mesh);
    call bio_files%def_node_var('BenSi',   'Benthos Silicate', 'mmol/m3',   Benthos(:,3), mesh);
    call bio_files%def_node_var('BenCalc', 'Benthos Calcite',  'mmol/m3',   Benthos(:,4), mesh);
    call bio_files%def_node_var('HPlus',   'Conc. of H-plus ions in the surface water', 'mol/kg',   GloHplus, mesh);
    if (ciso) then
        call bio_files%def_node_var('BenC_13',        'Benthos Carbon-13',   'mmol/m3',   Benthos(:,5), mesh);
        call bio_files%def_node_var('BenCalc_13',     'Benthos Calcite-13',  'mmol/m3',   Benthos(:,6), mesh); 
        if (ciso_14 .and. ciso_organic_14) then
            call bio_files%def_node_var('BenC_14',    'Benthos Carbon-14',   'mmol/m3',   Benthos(:,7), mesh);
            call bio_files%def_node_var('BenCalc_14', 'Benthos Calcite-14',  'mmol/m3',   Benthos(:,8), mesh); 
        end if ! ciso_14
    end if   ! ciso
    if (use_atbox) then
        call bio_files%def_node_var('xCO2',              'Atm. CO2 mixing ratio',   'mol / mol', x_co2atm(:),    mesh);
        if (ciso) then
            call bio_files%def_node_var('xCO2_13',       'Atm. 13CO2 mixing ratio', 'mol / mol', x_co2atm_13(:), mesh);
            if (ciso_14) then
                call bio_files%def_node_var('xCO2_14',   'Atm. 14CO2 mixing ratio', 'mol / mol', x_co2atm_14(:), mesh);
                call bio_files%def_node_var('cosmic_14', 'Cosmic 14C production',   'mol / s',   cosmic_14(:),   mesh);
            end if
        end if
    end if  ! use_atbox
  ...
  do j=num_tracers-bgc_num,num_tracers
  	 write(trname,'(A3,i4.4)') 'tra', j		 ! OG i1 -> i4
         write(longname,'(A15,i4.4)') 'passive tracer ', j
         units='none'
         call bio_files%def_node_var(trim(trname),        trim(longname), trim(units), tr_arr(:,:,j),   mesh) ! I use oid to write into restart.oce.nc OG
!        longname=trim(longname)//', Adams–Bashforth'
         call bio_files%def_node_var(trim(trname)//'_AB', trim(longname), trim(units), tr_arr_old(:,:,j), mesh)
  end do
...
! kh 31.03.22
  ...
  logical dumpfiles_exist, rawfiles_exist, binfiles_exist
...
#if defined(__recom)
      if (use_REcoM) call read_restart(bio_path, bio_files)
#endif
...
#if defined(__recom)
    if (use_REcoM) call write_restart(bio_path, bio_files, istep)
#endif
  end if
...
#if defined(__recom)
  if (use_REcoM) call write_raw_restart_group(bio_files, fileunit)
#endif
  close(fileunit)
...
#if defined(__recom)
    if (use_REcoM) call read_raw_restart_group(bio_files, fileunit)
#endif
    close(fileunit)
  else
